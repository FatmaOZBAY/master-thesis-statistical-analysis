...
dataset_person_period <- dataset_person_period[complete.cases(dataset_person_period[, 24:32]), ]
...
Y <- dataset_person_period[, 4:23] # matrix for response variables
X <- model.matrix(~ poly(Age, 9) + Sex + PMI + PSA + PC + FV + 
                    PA + Neglect + OPL + PIllness + EA, data = dataset_person_period)[, -1] # matrix for predictor variables

n <- 5381 # number of unique observation
...
Y <- dataset_person_period[, 4:23]
X <- model.matrix(~ poly(Age, 9) + Sex + PMI + PSA + PC + FV + 
                    PA + Neglect + OPL + PIllness + EA, data = dataset_person_period)[, -1]
...
Y <- dataset_person_period[, 4:23]
X <- model.matrix(~ poly(Age, 4) + Sex + PMI + PSA + PC + FV + PA + 
                     Neglect + OPL + PIllness, data = dataset_person_period)[, -1]
...
################################################################################
# We removed 1 predictor (PC) with lowest AIC/BIC scores
# So we should calculate the last model with "lpca" function to obtain the "implied"
# coefficients in the last model

# Final Model 
Y <- dataset_person_period[, 4:23]
X <- model.matrix(~ poly(Age, 4)  + Sex + PMI + PSA + FV + PA + Neglect + OPL 
                  + PIllness + EA, data = dataset_person_period)[, -1]
S <- 3
...
L_0r <- NA
for(i in 4:23){
  output_0r <- glm(dataset_person_period[, i] ~ 1, data = dataset_person_period, family = binomial(link = "logit"))
  L_0r[i-3] <- output_0r$deviance
}
names(L_0r) <- colnames(dataset_person_period[, 4:23])
L_0r
...
L_lr <- NA
for (i in 4:23) {
  output_lr <- glm(dataset_person_period[, i] ~ poly(Age, 4)  + Sex + PMI + PSA + FV + 
                     PA + Neglect + OPL + PIllness + EA, data = dataset_person_period, family = binomial(link = "logit"))
  L_lr[i-3] <- output_lr$deviance
}
names(L_lr) <- colnames(dataset_person_period[, 4:23])
L_lr
