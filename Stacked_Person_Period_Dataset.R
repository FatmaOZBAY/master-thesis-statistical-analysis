...
################################################################################
# This version is like that: there, the current age of disorder is used for the upper bound
...
for (patient in 1:nrow(twenty_disorders)) {
  ...
  if (is.na(m)) {
    ...
    for (i in 1:20) {
      empty_patient_3 <- matrix(nrow = n - 3, ncol = 4)
      empty_patient_3[, 1] <- twenty_disorders[patient, 1]
      empty_patient_3[, 2] <- 4:n
      empty_patient_3[, 3] <- 0
      empty_patient_3[, 4] <- colnames(twenty_disorders)[i + 2]
    ...
    for (j in 3:ncol(twenty_disorders)) {
      if (!is.na(twenty_disorders[patient, j]) && twenty_disorders[patient, j] > 0) {
        empty_patient_4 <- matrix(nrow = n - 3, ncol = 4)
        ...
        empty_patient_4[, 2] <- seq(from = 4, to = n)
        empty_patient_4[, 4] <- colnames(twenty_disorders)[j]
        empty_patient_4_list[[j - 2]] <- empty_patient_4
      } else {
        empty_patient_4_list[[j - 2]] <- matrix(c(rep(twenty_disorders[patient, 1], n - 3), 
                                                  4:n, 
                                                  rep(0, n - 3), 
                                                  rep(colnames(twenty_disorders)[j], n - 3)),
                                                ncol = 4)
      }
    }
...
merged_CAs_and_disorders_V2 <- merge(last_frame, merged_CAs_with_NAs, by = "CASEID", all = TRUE)

dataset_stacked_person_period <- merge(merged_CAs_and_disorders_V2, main_data_NCS[, c("CASEID", "Sex")], by = "CASEID", all = TRUE)

# Convert "Sex" variable into factor
dataset_stacked_person_period$Sex <- as.factor(dataset_stacked_person_period$Sex)

# 
dataset_stacked_person_period <- dataset_stacked_person_period %>%
  select("CASEID", "Age", "Sex", everything())


# Save the data set
save(dataset_stacked_person_period, file = "dataset_stacked_person_period.Rdata")
